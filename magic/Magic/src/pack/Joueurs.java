...
public class Joueurs {
	...
	private Cartes deckJ1[] = {Creatures.ID_1_1, Creatures.ID_2_1, Creatures.ID_3_1,
			...
			Creatures.ID_5_1, Creatures.ID_6_1, Sorts.ID_7_1, Sorts.ID_8_1}; //Le deck du joueur 1 est stocké ici
	...
	private Cartes deckJ2[] = {Creatures.ID_1_2, Creatures.ID_2_2, Creatures.ID_3_2,
			...
			Creatures.ID_5_2, Creatures.ID_6_2, Sorts.ID_7_2, Sorts.ID_8_2}; //Le deck du joueur 2 est stocké ici
	...
	public Pouvoirs getPh() {
		return ph;
	}

	public void setPh(Pouvoirs ph) {
		this.ph = ph;
	}
	...
	public static Joueurs J1 = new Joueurs("J1", "J1_Default", 30, 0);
	public static Joueurs J2 = new Joueurs("J2", "J2_Default", 30, 0);
	
	/**
	 * Cette méthode sert à faire piocher les joueurs
	 * 
	 */
	public void Pioche(){
		if(this.identifiant == "J1" && this.getNombreCartes() < 10 && Plateau.isEnJeu() == true){
			
			boolean havePick = false; //On défini qu'il n'a pas encore pioché

			while(havePick == false){ //Tant qu'il n'a pas pioché
				Random rand = new Random(); //On génère un nombre aléatoire
				int nombre = rand.nextInt(16); //Entre 0 et 15
				if(this.deckJ1[nombre].getEtat() == 1){ //Si le nombre est une carte dans le deck
			
			this.deckJ1[nombre].setEtat(2); //On la pioche
			int pio = this.getNombreCartes() + 1; //On ajoute une carte à la main du joueur spécifié
			...
			havePick = true; //On défini que le joueur a pioché
				}
			}
		}
		else if(this.identifiant == "J1" && this.getNombreCartes() >= 10 && Plateau.isEnJeu() == true){
			boolean havePick = false; //On défini qu'il n'a pas encore pioché
			...
			this.deckJ1[nombre].setEtat(0);//On la détruit
			System.out.println("\nMAGIC-LOG > PLAYER "+this.getName()+" HAVE 10 CARDS IN HIS HAND");
			System.out.println("MAGIC-LOG > PLAYER "+this.getName()+" DESTROYED > "+this.deckJ1[nombre].getName());
		...
		if(this.identifiant == "J2" && this.getNombreCartes() < 10 && Plateau.isEnJeu() == true){
				...
				boolean havePick2 = false; //On défini qu'il n'a pas encore pioché

				while(havePick2 == false){ //Tant qu'il n'a pas pioché
					Random rand2 = new Random(); //On génère un nombre aléatoire
					int nombre2 = rand2.nextInt(16); //Entre 0 et 15
					if(this.deckJ2[nombre2].getEtat() == 1){ //Si le nombre est une carte dans deck
						
				this.deckJ2[nombre2].setEtat(2);//On la pioche
				int pio2 = this.getNombreCartes() + 1; //On ajoute une carte à la main du joueur spécifié
				this.setNombreCartes(pio2);
				System.out.println("\nMAGIC-LOG > PLAYER "+this.getName()+" PICK UP > "+this.deckJ2[nombre2].getName());
				System.out.println("MAGIC-LOG > PLAYER "+this.getName()+" HAVE "+this.getNombreCartes()+ " CARDS NOW");
				
				havePick2 = true; //On défini que le joueur a pioché
				};
			}
		}
		else if(this.identifiant == "J2" && this.getNombreCartes() >= 10 && Plateau.isEnJeu() == true){
			boolean havePick2 = false; //On défini qu'il n'a pas encore pioché
			...
			this.deckJ2[nombre2].setEtat(0);//On la détruit
			System.out.println("\nMAGIC-LOG > PLAYER "+this.getName()+" HAVE 10 CARDS IN HIS HAND");
			System.out.println("MAGIC-LOG > PLAYER "+this.getName()+" DESTROYED > "+this.deckJ2[nombre2].getName());
	...
	/**
 	* Cette méthode fait mourir les joueurs lorsqu'ils ont 0 points de vie.
 	* 
 	*/
	public void Meurt(){
	...
	/**
	 * @param id 
	 * ID du joueur à qui on passe la main
	 * 
	 * Méthode qui passe le tour des joueurs
	 *
	 */
	public void Passe(Joueurs id){
		if(this.identifiant == "J1" && this.isJoue() == true && this.vie >= 0 && Plateau.isEnJeu() == true){ //On vérifie si c'est bien 
			...
			this.Meurt(); //On vérifie s'il n'y a pas de joueurs morts 
			id.Meurt();
		...
		for(int i = 0 ; i < id.deckJ2.length ; i++) //On vérifie les 30 cartes du deck
		{
			if(id.deckJ2[i].getType() == 1  && id.getDeckJ2()[i].getEtat() == 3){ //Si ce sont des créatures
				id.deckJ2[i].setPeutFrapper(true); //On défini qu'elle peuvent à nouveau frapper
					...
					Pouvoirs.P1.setUtilisable(true); //On défini qu'ils peuvent être à nouveau utilisés 
					Pouvoirs.P2.setUtilisable(true); //On défini qu'ils peuvent être à nouveau utilisés 
					Pouvoirs.P3.setUtilisable(true); //On défini qu'ils peuvent être à nouveau utilisés 
					Pouvoirs.P4.setUtilisable(true); //On défini qu'ils peuvent être à nouveau utilisés 
					Pouvoirs.P5.setUtilisable(true); //On défini qu'ils peuvent être à nouveau utilisés 
					Pouvoirs.P6.setUtilisable(true); //On défini qu'ils peuvent être à nouveau utilisés 
					Pouvoirs.P7.setUtilisable(true); //On défini qu'ils peuvent être à nouveau utilisés 
				//	Pouvoirs.P8.setUtilisable(true); //On défini qu'ils peuvent être à nouveau utilisés 
					Pouvoirs.P9.setUtilisable(true); //On défini qu'ils peuvent être à nouveau utilisés 
					Pouvoirs.P10.setUtilisable(true); //On défini qu'ils peuvent être à nouveau utilisés 
					Pouvoirs.P11.setUtilisable(true); //On défini qu'ils peuvent être à nouveau utilisés 
		}
	};
		if(this.identifiant == "J2" && this.isJoue() == true && this.vie >= 0 && Plateau.isEnJeu() == true){ //On vérifie si c'est bien 
			// son tour, qu'il est vivant et bien entendu que le plateau est occupé
	...
	/**
	 * Méthode d'initialisation du jeu, à mettre dans une autre classe
	 * 
	 */
	...
	/**
	 * Méthode de réinitialisation, à mettre dans une autre classe
	 * 
	 */
